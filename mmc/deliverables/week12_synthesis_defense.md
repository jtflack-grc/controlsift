...
4. **Did TF-IDF beat Gemma?** The published values suggest a large difference, but they are from different dataset versions. Classical experiments use v1.1.0 and Gemma uses v1.0.0, so ControlSift does not claim a controlled cross-family win.  
...
6. **What was the most important research lesson?** A benchmark must earn its difficulty, fine-tuning does not guarantee improvement, and an honest negative result is more useful than manufacturing an AI success story.
