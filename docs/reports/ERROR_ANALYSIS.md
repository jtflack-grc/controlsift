...
Within the controlled Gemma v1.0 experiments, **few-shot prompting is strongest**. QLoRA doesn't beat few-shot and introduces a substantial output-contract failure: fewer than half of test responses parse into a valid target label. That formatting reliability issue is treated as part of model performance rather than silently repaired away.

The small numeric difference between zero-shot and QLoRA isn't presented as a meaningful fine-tuning gain. The stronger conclusion is that the attempted QLoRA adaptation didn't improve on few-shot prompting and materially worsened label-output reliability.
...
`scripts/build_failure_lab.py` and the public Failure Lab preserve representative mistakes and difficult slices. `scripts/run_human_review.py` provides structural review receipts for the synthetic challenge data; this isn't represented as an independently human-labeled gold standard.

## Research conclusion

The error analysis supports a bounded conclusion rather than a winner-takes-all leaderboard. Dataset hardening exposed how easily a synthetic benchmark can reward shortcuts. The completed Gemma runs showed that fine-tuning didn't automatically improve classification and that output reliability can dominate usefulness. The dataset-version mismatch discovered during final review is explicitly retained as a reproducibility limitation instead of being hidden by an unsupported cross-family comparison.
